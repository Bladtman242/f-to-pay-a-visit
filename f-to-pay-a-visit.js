...
const keypressHandler_old = (e) => {
  // DFA: Relevance check could be input, preprocessed I'm not sure what to do
  // with blur. It shouldn't be necessary, but clearly some wobsites prevent
  // the regular browser handling of this?
  //
  // Ignore key presses we know have nothing to do with us
  if (!eventIsRelevant(e)) {
    if(e.key === 'Escape') {
      document.activeElement.blur();
    }
    console.log('event irrelevant', e);
    return;
  }

  const key = e.key;

  //DFA: This could be handled as considering the 500ms passing an input,
  //causing a state change, or, perhaps more conveniently, this could be done
  //as a pre-processing step, occurring outside of the state machine
  //
  // check if ',' has been pressed recently, we use this as an escape, to allow
  // users to use f and Escape to things other than using this extension
  if (watchMap.contains500ms(pressedKeys, ',')) {
    watchMap.delete(pressedKeys, ',');
    console.log('event escaped');
    return;
  }

  // escape future key press
  if (key === ',' && !displayIsActive()) {
    // DFA: escape/, should be handled in accordance to how escaping the the
    // escape state after 500ms is handled (see above).
    console.log('escape next event');
    watchMap.add(pressedKeys, ',');
  } else if (key === 'Escape' && displayIsActive()) {
    // DFA: displayActive is a state (or multiple states), escape is an input
    console.log('clear');
    clear();
  } else if (hasModKey(e) && !displayIsActive()) {
    // DFA: This will be different, as we will allow modifier keys, though
    // perhaps not on the inactive->active transition? Regardless, modifiers
    // will be part of either the input, or processing.
    console.log('event takes precedence', e);
    return;
  } else if (displayIsActive()) {
    // DFA: displayactive will be state, the input (e here) will be input
    console.log('filter');
    filter(e);
  } else if (key === 'f' && !displayIsActive()) {
    // DFA: active is state, f is input
    console.log('display');
    clear();
    display();
  } else {
    // DFA: This is essentially when an input comes, for which the current
    // state has no outgoing transitions
    console.log('event processed, without an action', e);
    return;
  }

  // DFA: This will be part of the individual transition side effects
